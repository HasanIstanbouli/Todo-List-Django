...
from Todo_List_App.models import Category, Notifications, PasswordResetRequest, Profile, Task

from .forms import CustomSetPasswordForm, PasswordChangeForm, ProfileForm, SignUpForm
from .tasks import create_notification, send_welcome_email
        ...
        if form.is_valid():
            ...
            if created:
                profile_form = ProfileForm(
                    request.POST, request.FILES, instance=profile, user_instance=user
                )
            ...
            messages.success(
                request,
                "Verify email to receive reminder & reset password",
...
def send_verification_email(request, user_id):
    ...
    limit_reached, remaining_time = PasswordResetRequest.email_verification_limit_reached(
        user, verification_limit, verification_duration
    )
    if limit_reached:
        if remaining_time is not None:
            messages.error(
                request,
                "Maximum email verification requests reached. "
            ...
            messages.error(
                request,
                "Maximum email verification requests reached. Please try again later.",
                extra_tags="profileInfoError",
            )
        return redirect("profile")
    # Create a new email verification request
    reset_request = PasswordResetRequest(user=user, email_verification_timestamp=timezone.now())
    reset_request.save()
    token = default_token_generator.make_token(user)
    uid = urlsafe_base64_encode(force_bytes(user.encrypted_id))
    verification_link = (
        request.build_absolute_uri(reverse("verify_email")) + f"?uid={uid}&token={token}"
    )
...
def findAccount(request):
    ...
    if request.method == "POST":
        ...
        if not user:
            ...
            messages.error(
                request, "User with this Email doesn't exist", extra_tags="findAccountError"
            )
            return render(
                request,
                "findAccount.html",
            ...
            return render(
                request, "findAccount.html", {"user": user, "emailVerified": user.email_verified}
            )
...
def sendResetLink(request):
    if request.method == "POST":
        ...
        limit = 5
        duration = 60
        limit_reached, remaining_time = PasswordResetRequest.request_limit_reached(
            user, limit, duration
        )
        if limit_reached:
            messages.error(
                request,
                "Maximum password reset requests reached. "
                f"Please try again in {remaining_time} minutes.",
        ...
        PasswordResetRequest.objects.filter(user=user).update(
            reset_link_used=True, reset_link_generated_at=None, reset_link_expiry=None
        )
        ...
        reset_link = (
            request.build_absolute_uri(reverse("checkResetLink")) + f"?uid={uid}&token={token}"
        )
        reset_html = f'<a href="{reset_link}">Reset Password</a>'
        message = (
            f"Please click the link below to reset your password: {reset_html}. "
        ...
        try:
            ...
            if "Connection unexpectedly closed" in errorMessage:
                errorMessage = (
                    "There was a problem connecting to the email server. Please try again later."
                )
            elif "Authentication" in errorMessage:
                errorMessage = (
                    "Authentication with the email server failed. Please contact support."
                )
            else:
...
def checkResetLink(request):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        if (
            ...
            or reset_request.reset_link_used
            or reset_request.reset_link_expiry < timezone.now()
        ):
            messages.error(
                request, "This reset link is invalid or expired.", extra_tags="resetLinkError"
            )
            return redirect("findAccount")
...
def invalidate_session(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "failed", "error": "No password reset request found for this user"}
        )
    except CustomUser.DoesNotExist:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                if "Connection unexpectedly closed" in errorMessage:
                    errorMessage = (
                        "There was a problem connecting to the email server. "
            ...
            return render(
                request,
                "resetPassword.html",
                {"form": form, "user_id": user_id, "remaining_time": int(remaining_time)},
            )
    else:
    ...
    if request.method == "POST":
        ...
        if "profilePicture" in request.FILES or any(field in request.POST for field in form.fields):
            if form.is_valid():
                if not form.has_changed():
                    messages.success(
                        request, "Profile is unchanged", extra_tags="profileInfoSuccess"
                    )
                else:
                    ...
                    notifications.save()
                    messages.success(
                        request, "Profile updated successfully", extra_tags="profileInfoSuccess"
                    )
            else:
        ...
        if any(field in request.POST for field in password_form.fields):
            if password_form.is_valid():
                ...
                messages.success(
                    request, "Password changed successfully", extra_tags="profilePassSuccess"
                )
...
def create_task(request):
    ...
    if request.method == "POST":
        ...
        if due_datetime <= min_due_datetime:
            notifications = Notifications(
                name="Due datetime must be at least 5 minutes from now. "
                f'You entered {due_datetime.strftime("%Y-%m-%d | %H:%M")}',
                date=timezone.now(),
                user=request.user,
            )
            notifications.notificationsCount += 1
...
def running_tasks(request):
    ...
    tasks_queryset = Task.objects.filter(user=request.user).filter(
        Q(status=Task.PENDING) | Q(status=Task.OVERDUE)
    )
    ...
    if not is_refreshed:
        ...
        if not task_count and search_query:
            messages.error(
                request, f'Task "{search_query}" wasn\'t found.', extra_tags="runningTaskError"
            )
        elif task_count and search_query:
            messages.success(
                request,
                f"[{task_count}] tasks found matching your search query.",
                extra_tags="runningTaskSuccess",
            )
        elif not task_count and not search_query:
            messages.error(
                request, "You haven't created any tasks yet.", extra_tags="runningTaskError"
            )
...
def completed_tasks(request):
    ...
    if not is_refreshed:
        ...
        if not task_count and search_query:
            messages.error(
                request, f'Task "{search_query}" wasn\'t found.', extra_tags="completedTaskError"
            )
        elif task_count and search_query:
            messages.success(
                request,
            ...
            messages.error(
                request, "You haven't completed any tasks yet.", extra_tags="completedTaskError"
            )
    context = {
...
def clear_history(request):
    ...
    messages.success(
        request, "Completed tasks history has been cleared.", extra_tags="completedTaskSuccess"
    )
    return redirect("completed_tasks")
...
def delete_category(request, category_id):
    ...
    if categoryName == "Others":
        ...
        messages.success(
            request, "Tasks in Others category deleted successfully", extra_tags="categorySuccess"
        )
        notifications = Notifications(
...
def delete_account(request, user_id):
    ...
    if user.is_superuser:
        ...
        messages.error(
            request,
            "Admin account can only be deleted from the admin page.",
            extra_tags="profileInfoError",
        )
        return redirect("profile")
    ...
    if request.method == "POST":
        ...
        if confirm == "CONFIRM" and user.check_password(password):
            ...
            messages.error(
                request,
                "Confirmation text or password is incorrect.",
                extra_tags="profileInfoError",
            )
