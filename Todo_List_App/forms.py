...
from Todo_List_App.models import CustomUser, Profile
...
class PasswordChangeForm(DjangoPasswordChangeForm):
    def is_empty(self):
        return not (
            ...
            or self.cleaned_data.get("new_password1")
            or self.cleaned_data.get("new_password2")
        )
        ...
        if old_password or new_password1 or new_password2:
            ...
            if new_password1 and new_password2:
                ...
                if len(new_password1) < 8:
                    self.add_error(
                        "new_password1", "The new password must be at least 8 characters long."
                    )

                if not re.search(r"\d", new_password1):
                    self.add_error(
                        "new_password1", "The new password must contain at least one number."
                    )
                ...
                if old_password and new_password1 == old_password:
                    self.add_error(
                        "new_password1", "The new password cannot be the same as the old password."
                    )
